...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      if (user) {
        // Busca os pedidos do Rafael no banco de dados
              ...
              {orders.map((order) => (
                <div key={order.id} className="bg-[#303030] p-8 rounded-[2rem] border border-white/5 shadow-2xl hover:border-cintelYellow/30 transition-all group">
                  ...
                  <Link 
                    ...
                    className="w-full flex items-center justify-center bg-white text-[#1a1a1a] py-4 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-cintelYellow transition-all"
                  >
                    Acessar Dashboard
                  </Link>
                </div>
