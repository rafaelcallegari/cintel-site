...
export default function BlogPage() {
  ...
  return (
    <section className="bg-white min-h-screen pt-32 pb-20 font-alegreya">
      <div className="max-w-6xl mx-auto px-6">
        
        {/* Cabeçalho */}
        <div className="flex justify-between items-end mb-16 border-b border-gray-100 pb-8">
          <div>
            <h1 className="text-5xl font-extrabold text-[#303030] mb-4">Insights Cintel</h1>
            <p className="text-gray-500 text-xl italic">Ciência, dados e a transformação do espaço urbano.</p>
          </div>
          {isAdmin && (
            <button onClick={() => setShowAdmin(!showAdmin)} className="text-[10px] font-bold text-gray-400 hover:text-cintelYellow transition-colors uppercase tracking-[0.3em]">
              {showAdmin ? 'Fechar Editor' : 'Nova Postagem'}
            </button>
          )}
        </div>
        ...
        {showAdmin && isAdmin && (
          <div className="mb-20 bg-gray-50 p-10 rounded-[2.5rem] border-2 border-dashed border-cintelYellow">
            <h2 className="text-2xl font-bold mb-6 text-[#303030]">Publicar Nova Análise</h2>
            <form onSubmit={handlePublish} className="space-y-4">
              ...
              <div className="flex justify-between items-center pt-4">
                ...
                <button disabled={loading} type="submit" className="bg-[#303030] text-white px-10 py-3 rounded-full font-bold hover:bg-black transition-all uppercase text-xs tracking-widest">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {posts.map((post) => (
            <div key={post.id} className="relative group">
              {/* Botão de Excluir flutuante para Admin */}
              {isAdmin && (
                <button 
                  onClick={(e) => handleDelete(e, post.id)}
                  className="absolute top-4 right-4 z-20 bg-white/90 p-3 rounded-full text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-lg opacity-0 group-hover:opacity-100"
              ...
              <Link href={`/blog/${post.slug}`} className="cursor-pointer block">
                <article>
                  <div className="mb-6 overflow-hidden rounded-[2.5rem] bg-gray-100 aspect-video relative transition-transform duration-500 group-hover:scale-[1.01]">
                    <div className="absolute inset-0 flex items-center justify-center text-gray-300 font-bold uppercase tracking-tighter text-4xl opacity-20 italic">
                      Cintel Insights
                    </div>
                    <div className="absolute top-6 left-6 bg-cintelYellow text-[#303030] px-4 py-1 rounded-full text-[10px] font-black uppercase z-10 tracking-widest">
                      {post.categoria}
                    </div>
                  </div>
                  ...
                  <h2 className="text-3xl font-bold text-[#303030] mt-2 mb-4 group-hover:text-cintelYellow transition-colors leading-tight">
                    {post.titulo}
                  </h2>
                  <p className="text-gray-600 leading-relaxed text-lg line-clamp-2 italic">
                    {post.resumo}
                  </p>
